...
class CVDDFlowNet(BaseNet):
    ...
    def __init__(self, pretrained_model, attention_size=100, n_attention_heads=1):
        ...
        # flow models
        self.flow_model = Glow(self.hidden_size)
    ...
    def forward(self, x):
        ...
        # flow module(2D inputs)
        inputs_flow = torch.sum(M * context_weights.unsqueeze(2), dim=1)
        loss_mean, loss_batch = self.flow_model(inputs_flow)

        return cosine_dists, context_weights, A, loss_mean, loss_batch
