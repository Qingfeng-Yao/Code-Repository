...
from utils.misc import one_hot, create_T_one_hot, run_padded_LSTM, run_sequential_with_mask, create_embed_layer, mixture_log_cdf, safe_log, inverse_func, mixture_log_pdf, mixture_inv_cdf
from utils.distributions import LogisticDistribution
...
class LSTMFeatureModel(nn.Module):
	...
	def __init__(self, c_in, c_out, hidden_size, max_seq_len,
					   ...
					   input_dp_rate=0.0, use_time_embed=False, **kwargs):
		super().__init__()
	...
	def forward(self, x, length=None, channel_padding_mask=None, length_one_hot=None, **kwargs):
		...
		if self.use_time_embed:
			embed = self.time_concat(x=embed, length_one_hot=length_one_hot, length=length)
...
class AutoregressiveLSTMModel(nn.Module):
	...
	def __init__(self, c_in, c_out, hidden_size, max_seq_len, 
					   num_layers=1, dp_rate=0.0, 
					   input_dp_rate=0.0, use_time_embed=False,
					   direction=0, **kwargs):
		super().__init__()
		self.lstm_model = LSTMFeatureModel(c_in, c_out, hidden_size, num_layers=num_layers,
						max_seq_len=max_seq_len,
						dp_rate=dp_rate, input_dp_rate=input_dp_rate, use_time_embed=use_time_embed)
...
def create_LCE_flows(num_dims, embed_dims, encoding_params):
	num_flows = encoding_params["num_flows"]
	num_hidden_layers = encoding_params["hidden_layers"]
	hidden_size = encoding_params["hidden_size"]
...
class LinearCategoricalEncoding(FlowLayer):
	...
	def __init__(self, num_dimensions,
					   vocab_size=-1, vocab=None, word_vectors=None,
					   encoding_params=None,
					   default_embed_layer_dims=8,
					   category_prior=None):
		super().__init__()
		self.D = num_dimensions
		self.embed_layer, self.vocab_size = create_embed_layer(vocab_size, vocab, word_vectors, default_embed_layer_dims)
		...
		self.flow_layers = create_LCE_flows(num_dims=num_dimensions, 
							embed_dims=self.embed_layer.weight.shape[1], encoding_params= encoding_params)
