...
class Text8Dataset(data.Dataset):
	...
	def __init__(self, max_seq_len, train=False, val=False, test=False, root="data/", **kwargs):
		...
		print("Number of total tokens", len(self.data))
		print("Dataset length(num of sents)", len(self))
...
def prepare_text8(root="../data/"):
	...
	vocabulary = {t: i for i, t in enumerate(unique_tokens)} # # key: token, value: index
...
if __name__ == '__main__':
	...
	for data_index in range(10):
		sents = data_loader.next() # a tuple, first element shape [batch_size, max_len](dtype int), second element shape [batch_size, ]; text8 is not padded
		print(sents)
		if data_index > 4:
			break
