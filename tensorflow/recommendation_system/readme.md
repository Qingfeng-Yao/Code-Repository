...
- amazon(eletronics)
    ...
    - 生成和正样本一样数目的负样本，并按用户进行分组；每个训练集和测试集中的每一个样本是一个元组，包含用户id、历史序列、itemid以及是否评论(0或1)；dataset.pkl由train_set和test_set组成，前者共有2608764，后者共有384806
    - 最后生成amazon_train.tfrecords和amazon_valid.tfrecords
- DIN
