...
from embedflow.general.mutils import get_param_val, append_in_dict, get_device, create_channel_mask
from embedflow.general.parameter_scheduler import *
from embedflow.layers.flows.distributions import create_prior_distribution

from embedflow.experiments.language_modeling.lstm_model import LSTMModel
from embedflow.experiments.language_modeling.datasets.text8 import Text8Dataset
from embedflow.experiments.language_modeling.datasets.penntreebank import PennTreeBankDataset
from embedflow.experiments.language_modeling.datasets.wikitext import WikiTextDataset
...
class TaskLanguageModeling(TaskTemplate):
	...
	def _train_batch_rnn(self, x_in, x_length, x_channel_mask, **kwargs):
		...
		# loss, _, _ = self._calc_loss(logprob.zeros_like(), -logprob, x_length)
