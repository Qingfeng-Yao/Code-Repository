...
# def create_encoding(args, dataset_class, vocab=None, vocab_size=-1, category_prior=None):
...
# 	use_dequantization = args.encoding_dequantization
# 	use_variational = args.encoding_variational
...
# def vd_create_flows(config, embed_dims):
# 	num_flows = util.get_param_val(config, "num_flows", 4)
# 	model_func = util.get_param_val(config, "model_func", allow_default=False)
# 	block_type = util.get_param_val(config, "block_type", None)
...
# def lce_create_flows(num_dims, embed_dims, config):
# 	num_flows = util.get_param_val(config, "num_flows", 0)
# 	num_hidden_layers = util.get_param_val(config, "hidden_layers", 2)
# 	hidden_size = util.get_param_val(config, "hidden_size", 256)
	
# 	# We apply a linear net in the coupling layers for linear flows
# 	block_type_name = "LinearNet"
# 	block_fun_coup = lambda c_out : util.LinearNet(c_in=num_dims,
# 											  c_out=c_out,
...
# 	block_fun_actn = lambda : util.SimpleLinearLayer(c_in=embed_dims, c_out=2*num_dims, data_init=True)
...
# def vce_create_flows(num_dims, embed_dims, config):
# 	num_flows = util.get_param_val(config, "num_flows", 0)
# 	model_func = util.get_param_val(config, "model_func", allow_default=False)
# 	block_type = util.get_param_val(config, "block_type", None)
# 	num_mixtures = util.get_param_val(config, "num_mixtures", 8)
...
# 			nn_out = util.run_sequential_with_mask(self.nn, x, 
# 											  length=length, 
# 											  **kwargs)
# 		else:
...
# 			z_out = util.mixture_log_cdf(x=orig_z, prior_logits=log_pi, means=mixt_t, log_scales=mixt_log_s).exp()
...
# 			z_out, mixt_ldj = util.inverse(z_out)
# 			# Output affine transformation
# 			z_out = (z_out + t) * log_s.exp()
# 			# Determine LDJ of the transformation
# 			logistic_ldj = util.mixture_log_pdf(orig_z, prior_logits=log_pi, means=mixt_t, log_scales=mixt_log_s)
# 			# Combine all LDJs
...
# 			z_out, mixt_ldj = util.inverse(z_out, reverse=True)
...
# 			z_out = util.mixture_inv_cdf(z_out, prior_logits=log_pi, means=mixt_t, log_scales=mixt_log_s)
# 			# Determine LDJ of this transformation
# 			logistic_ldj = util.mixture_log_pdf(z_out, prior_logits=log_pi, means=mixt_t, log_scales=mixt_log_s)
...
# 		self.embed_layer, self.vocab_size = util.create_embed_layer(vocab, vocab_size, default_embed_layer_dims)
# 		self.num_categories = self.vocab_size
	
# 		self.prior_distribution = util.LogisticDistribution(mu=0.0, sigma=1.0) # Prior distribution in encoding flows
# 		self.flow_layers = lce_create_flows(num_dims=num_dimensions, 
# 										 embed_dims=self.embed_layer.weight.shape[1], 
# 										 config=flow_config)
# 		# Create decoder if needed
# 		if self.use_decoder:
# 			self.decoder = util.create_decoder(num_categories=self.vocab_size, 
# 										   num_dims=self.D,
...
# 		orig_class_mask = util.c_one_hot(z_categ.squeeze(), num_classes=log_prob_denominator.size(1))
...
# 		super().__init__()
# 		self.embed_layer, self.vocab_size = util.create_embed_layer(vocab, vocab_size, default_embed_layer_dims)
# 		self.flow_layers = vd_create_flows(flow_config, self.embed_layer.weight.shape[1])
...
# 		self.prior_distribution = util.LogisticDistribution(mu=0.0, sigma=1.0) # Prior distribution in encoding flows
# 		self.flow_layers = vce_create_flows(num_dims=num_dimensions, 
# 										 embed_dims=self.embed_layer.weight.shape[1], 
# 										 config=flow_config)
# 		# Create decoder if needed
# 		self.decoder = util.create_decoder(num_categories=self.vocab_size, 
...
# 	def __init__(self, args, dataset_class, vocab_size, vocab):
...
# 		self.latent_dim = self.args.encoding_dim
# 		model_func = lambda c_out : AutoregressiveLSTMModel(c_in=self.latent_dim,
# 															c_out=c_out,
# 															max_seq_len=self.max_seq_len,
# 															num_layers=self.args.coupling_hidden_layers,
# 															hidden_size=self.args.coupling_hidden_size,
# 															dp_rate=self.args.coupling_dropout,
# 															input_dp_rate=self.args.coupling_input_dropout)
# 		self.args.categ_encoding_flow_config_model_func = model_func
# 		self.encoding_layer = create_encoding(self.args, 
# 											  dataset_class=self.dataset_class, 
# 											  vocab_size=self.vocab_size,
# 											  vocab=self.vocab)

# 		num_flows = self.args.coupling_num_flows
...
# 						num_mixtures=self.args.coupling_num_mixtures)
# 			]
...
# 			kwargs["src_key_padding_mask"] = util.create_transformer_mask(length)
# 			kwargs["channel_padding_mask"] = util.create_channel_mask(length)
...
# 				kwargs["src_key_padding_mask"] = util.create_transformer_mask(kwargs["length"])
# 				kwargs["channel_padding_mask"] = util.create_channel_mask(kwargs["length"])
# 			for layer_index, layer in enumerate(self.flow_layers):
# 				batch_list = FlowModel.run_data_init_layer(batch_list, layer)
...
# 		lstm_out = util.run_padded_LSTM(x=embed, lstm_cell=self.lstm_module, length=length)
...
# 		super().__init__()
# 		if model_params is not None:
# 			hidden_size = util.get_param_val(model_params, "coupling_hidden_size", hidden_size)
# 			embedding_dim = hidden_size//4
# 			num_layers = util.get_param_val(model_params, "coupling_hidden_layers", num_layers)
# 			dp_rate = util.get_param_val(model_params, "coupling_dropout", dp_rate)
# 			input_dp_rate = util.get_param_val(model_params, "coupling_input_dropout", input_dp_rate)
# 			max_seq_len = util.get_param_val(model_params, "max_seq_len", max_seq_len)
...
# 	def _determine_logp(self, x, length=None, channel_padding_mask=None, **kwargs):
# 		if length is not None and channel_padding_mask is None:
# 			channel_padding_mask = util._create_length_mask(length, max_len=x.shape[1])
# 		x_embed = self.embeddings(x)
# 		if self.time_concat is not None:
# 			x_embed = self.time_concat(x=x_embed, length=length)
# 		init_state = self.init_state.expand(-1, x.size(0), -1).contiguous()
