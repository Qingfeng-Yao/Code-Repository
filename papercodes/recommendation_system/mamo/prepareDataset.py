...
def data_generation(dataset='movielens'):
    ...
    if processing_code == 1:
        ...
        for u_id in tqdm(u_all_ids):
            ...
            pickle.dump(u_feature, open('{}/{}/raw/'.format(storing_path, dataset)+'sample_'+str(file_index)+'_x1.p', 'wb')) # x1是用户特征索引值
            pickle.dump(i_feature_file, open('{}/{}/raw/'.format(storing_path, dataset)+'sample_'+str(file_index)+'_x2.p', 'wb')) # x2是该用户对应的多个item的特征
            pickle.dump(ratings, open('{}/{}/raw/'.format(storing_path, dataset)+'sample_'+str(file_index)+'_y.p', 'wb'))  # y是评分
            pickle.dump(state_codes, open('{}/{}/raw/'.format(storing_path, dataset)+'sample_'+str(file_index)+'_y0.p', 'wb')) # y0指明用户和item的类型
