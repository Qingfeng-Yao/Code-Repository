...
def load_sparse_temporal_data(dataset_str, emb_str, f_dim, train=.825, val=.175, test=.0, shuffle=False):
    ...
    x, y, idx, tx, ty, tidx= tuple(objects)
    print("x samples:{} y {};tx samples:{} ty {}".format(len(x), len(y), len(tx), len(ty)))
    print(x.shape, y.shape, idx.shape, tx.shape, ty.shape, tidx.shape)
    print(torch.nonzero(torch.from_numpy(y)).size(0))
    print(torch.nonzero(torch.from_numpy(ty)).size(0))
    print(torch.nonzero(torch.from_numpy(y)==0).size(0))
    print(torch.nonzero(torch.from_numpy(ty)==0).size(0))
    ## train 
    for xx in x:
    ...
    for name in names_dict:
        ...
        val_dict[name] = [names_dict[name][i] for i in val_idx]
    test_dict = {'x':tx, 'y':ty, 'idx':tidx}
    emb_file = os.path.join('data/{}'.format(dataset_str), "{}.emb_{}".format(emb_str, f_dim))
    emb = load_w2v_emb(emb_file)
    print(emb.shape)
    emb = torch.FloatTensor(emb)
    return train_dict, val_dict, test_dict, emb
